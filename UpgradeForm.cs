...
namespace SpinARayan
{
    public partial class UpgradeForm : Form
    {
        ...
        private void UpdateRollCooldownInfo()
        {
            ...
            // Calculate next cooldown (halved, minimum 0.1s)
            double nextCooldown = Math.Max(0.1, currentCooldown / 2.0);
            ...
            btnReduceCooldown.Enabled = (_gameManager.AdminMode || _gameManager.Stats.Gems >= nextLevelCost) && currentCooldown > 0.1;
        ...
        {
            return 50 + (level * 500); // Jedes Level kostet 500 Gems mehr
        }
        ...
        private void btnReduceCooldown_Click(object sender, EventArgs e)
        {
            ...
            if ((_gameManager.AdminMode || _gameManager.Stats.Gems >= cost) && _gameManager.Stats.RollCooldown > 0.1)
            {
                if (!_gameManager.AdminMode)
                {
                ...
                // Halve the cooldown, minimum 0.1s
                _gameManager.Stats.RollCooldown = Math.Max(0.1, _gameManager.Stats.RollCooldown / 2.0);
